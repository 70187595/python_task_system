"""
Эксперимент 6: Обучение модели с меньшим количеством эпох (1000)
Цель: Проверить, достаточно ли 1000 эпох для качественного обучения
"""
...
def main():
    print("🧪 ЭКСПЕРИМЕНТ 6: epochs=1000")
    print("=" * 60)
    ...
    # Создаем сеть с меньшим количеством эпох
    ...
    print(f"   Epochs: 1000 (сокращено)")
    
    # Обучаем
    training_data = [(X[i:i+1], y[i:i+1]) for i in range(len(X))]
    print(f"\n🚀 Начинаем обучение...")
    
    history = network.train(training_data, epochs=1000)
    
    # Сохраняем модель
    os.makedirs('experiments/results', exist_ok=True)
    network.save_model('experiments/results/model_exp6_epochs1000.json')
    
    with open('experiments/results/history_exp6_epochs1000.json', 'w', encoding='utf-8') as f:
    ...
    print(f"\n✅ Модель сохранена: experiments/results/model_exp6_epochs1000.json")
    print(f"✅ История сохранена: experiments/results/history_exp6_epochs1000.json")
