"""
Эксперимент 3: Обучение модели с очень большим скрытым слоем (hidden_size=16)
Цель: Проверить влияние избыточной емкости модели на качество
"""
...
def main():
    print("🧪 ЭКСПЕРИМЕНТ 3: hidden_size=16")
    print("=" * 60)
    ...
    # Создаем сеть с очень большим скрытым слоем
    ...
    print(f"\n🔧 Параметры:")
    print(f"   Input: 10, Hidden: 16, Output: 3")
    ...
    network.save_model('experiments/results/model_exp3_hidden16.json')
    
    with open('experiments/results/history_exp3_hidden16.json', 'w', encoding='utf-8') as f:
    ...
    print(f"\n✅ Модель сохранена: experiments/results/model_exp3_hidden16.json")
    print(f"✅ История сохранена: experiments/results/history_exp3_hidden16.json")
